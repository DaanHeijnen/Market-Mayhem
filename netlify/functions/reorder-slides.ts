...
/** Reorder a PRESENTATIE round's content. The whole list, so the result is what the host sees. */
export default wrap(async request => {
  const admin = await requireAdmin(request);
  const p = await body<any>(request);
  const gameId = intValue(p.gameId, 'gameId', { min: 1 });
  const roundId = intValue(p.roundId, 'roundId', { min: 1 });
  const ids: number[] = Array.isArray(p.slideIds)
    ? p.slideIds.map((id: unknown, i: number) => intValue(id, `slideIds[${i}]`, { min: 1 }))
    : [];
  if (!ids.length) throw new HttpError(400, 'slideIds is required');
  ...
  return ok(await withTransaction(async client => {
    ...
    const count = await reorderRoundContent(client, 'presentation_slides', roundId, ids);
