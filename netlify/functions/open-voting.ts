...
export default wrap(async (request) => {
  ...
  return ok(await withTransaction(async (client) => {
    ...
    if (prediction.rows[0].status !== 'DRAFT') throw new HttpError(409, 'Prediction must be DRAFT');

    await client.query(
      `UPDATE predictions SET status='VOTING',voting_opened_at=NOW(),updated_at=NOW() WHERE id=$1`,
      [predictionId],
    );
    if (prediction.rows[0].visible_to_players) {
      await setScreenMode(client, gameId, 'PREDICTION_VOTING', admin.username, prediction.rows[0].round_id, predictionId);
    }
    await audit(client, gameId, admin.username, 'opened voting', 'prediction', predictionId);
