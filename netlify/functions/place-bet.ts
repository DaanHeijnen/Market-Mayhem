...
export default wrap(async (request) => {
  ...
  return ok(await withTransaction(async (client) => {
    ...
    // Lock the prediction before the wallet. Settlement also starts with the
    // prediction lock, so a bet racing with market close/settlement cannot
    // create a prediction<->wallet lock cycle.
    ...
    try {
      ...
      if ((error as { code?: string })?.code === '23505') throw new HttpError(409, 'You already placed a bet');
      throw error;
    }
