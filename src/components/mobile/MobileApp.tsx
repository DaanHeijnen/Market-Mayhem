...
export function MobileApp({ gameId }: { gameId: number }) {
  ...
  // A live Duolingo block owns the phone experience while it is the current round block.
  // This is backend-driven; URL/local state never decides whether the question is active.
...
function Shell({ children }: { children: any }) { return <main className="mobile-shell">{children}</main>; }
