...
export function RoundsPage({ state: s, gameId, roundId, run }: { state: any; gameId: number; roundId: number | null; run: RunMutation }) {
  ...
  return <div className="page-stack">
    ...
    {s.rounds.length === 0 ? <Empty title="No rounds yet — Create your first round" /> : <div className="card-list">
      {s.rounds.map((item: any) => <Card key={item.id}>
...
function RoundDetail({ state: s, round, run, back }: { state: any; round: any; run: RunMutation; back: () => void }) {
  ...
  return <div className="page-stack">
    ...
    {blocks.length === 0 ? <Empty title="No round content yet — Add the first block" /> : <div className="card-list">
      {blocks.map((block: any, index: number) => <Card key={block.id} className={s.game.current_round_block_id === block.id ? 'live-card' : ''}>
