  ...
  return <div className="page-stack">
    ...
    {!data ? <Empty title="Loading ledger…" /> : entries.length === 0 ? <Empty title="No transactions yet" /> : <>
      ...
      <Accordion title={`FULL DETAIL · ${entries.length} ENTR${entries.length === 1 ? 'Y' : 'IES'}`} open={detail} onToggle={() => setDetail(x => !x)}>
        ...
        <div className="table-wrap"><table><thead><tr><th>Timestamp</th><th>Player</th><th>Amount</th><th>Description</th><th>Type</th><th>Round</th><th>Group / question</th><th>Prediction</th><th>Roulette</th></tr></thead><tbody>{entries.map((x: any) => <tr key={x.id}><td>{new Date(x.created_at).toLocaleString()}</td><td>{x.display_name}</td><td className={x.amount >= 0 ? 'pos' : 'neg'}><b>{x.amount > 0 ? '+' : ''}{x.amount}</b></td><td>{x.description}</td><td><span className="mono">{x.transaction_type}</span></td><td>{x.round_number ? `R${String(x.round_number).padStart(2, '0')}` : '—'}</td><td>{x.group_name || x.question_prompt || '—'}</td><td>{x.prediction_number ? `#${x.prediction_number}` : '—'}</td><td>{x.roulette_game_id ? `#${x.roulette_game_id}` : '—'}</td></tr>)}</tbody></table></div>
      </Accordion>
    </>}
  </div>;
}
