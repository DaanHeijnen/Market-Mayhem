...
- a stored slot `SPINNING` outcome becomes `RESULT` after the reel animation window.
...
`total player value = wallet.current_balance + prediction locked + roulette locked + slot locked`.
...
Three reels of twelve symbols make exactly 1728 combinations. `slot_settings` holds the total probability pool, maximum spins per series and the stake bounds; `slot_outcomes` holds chances and payout multiplier per combination and is stored sparsely, because combinations with neither carry no meaning.

A configuration is usable only when all 36 reel symbols exist and the assigned chances equal the total pool exactly. `slotConfigurationStatus` is the single rule: Admin Settings renders it while typing and both player endpoints re-check it inside their transaction, so an incomplete or over-assigned table cannot be played.

```mermaid
flowchart LR
  M[Player Mobile] -->|stake, spins, SPIN| E[Game engine]
  E -->|weighted draw, payout, remaining spins| D[(PostgreSQL)]
  D --> S[Big Screen]
```

Reel artwork lives in `slot_symbols` as PNG bytes because Netlify Functions have no durable writable filesystem. A dedicated `/api/slot-symbol` endpoint serves them with a checksum-pinned immutable cache, which keeps 36 images out of every polled snapshot. Symbols are frozen while a series is live.

### Series and spin transaction

Locking a series locks game -> slot settings -> player -> existing series -> wallet. The whole series stake becomes a single `SLOT_DEPOSIT`, so `remaining_spins x stake_per_spin` is locked value for as long as spins are unused, exactly like a prediction deposit. A partial unique index permits one `ACTIVE` series per game night, because one slot machine is presented on one Big Screen. Admin can cancel an abandoned series, which refunds the unspun stake as `SLOT_REFUND`.

The SPIN command draws one roll with server-side cryptographic randomness over the configured pool and walks the ordered weighted table. The three reels are never rolled independently: the stored combination decides which symbol each reel shows. Payout is `round(stake x multiplier)` for that single spin and is credited as `SLOT_PAYOUT` in the same transaction that decrements the remaining spins.

The outcome is stored before animation begins. Player state masks the reel positions, multiplier and payout while the spin is `SPINNING`; Big Screen state carries them unmasked because it has to animate towards the stored result. Idempotency is deterministic per spin (`session id` + spins already used), so a retried SPIN can never draw twice.
...
Game reset requires Admin authentication, game ID and exact server-side phrase `yes delete`. It also removes slot reel artwork, the outcome distribution and every series/spin, and returns slot settings to their defaults. It is transactional, game-scoped, writes `GAME_RESET`, deletes game-owned operational/financial data and recreates dashboard state while leaving Admin sessions/audit history available.
